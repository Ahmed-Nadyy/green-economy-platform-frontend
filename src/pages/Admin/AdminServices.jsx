...
const AdminServices = () => {
  ...
  const ItemCard = ({ item, type }) => (
    <div className="relative overflow-hidden rounded-lg shadow-md group p-1 bg-[#e8e8e8]">
      <div className='bg-[#2ECC71] p-2 m-4 rounded-2xl'>
        <img src={item.image} alt={item.title} className="w-full h-24 object-contain" />
      </div>
      <div className="p-2">
        <h3 className="text-md text-gray-800 font-bold">{item.title}</h3>
      </div>
  ...
  return (
    <div className="rounded-lg shadow-md">
      <div className="mb-8 bg-white p-6 rounded-lg">
        <h2 className="text-2xl font-bold mb-4 text-gray-800">المحاصيل الزراعية</h2>
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
          ...
          <div 
            ...
            className="border-2 border-dashed border-gray-300 rounded-lg flex items-center justify-center h-40 bg-gray-50 hover:bg-gray-100 cursor-pointer transition-colors"
          >
      ...
      <div className="mb-8 bg-white p-6 rounded-lg">
        <h2 className="text-2xl font-bold mb-4 text-gray-800">مقالات التوعية</h2>
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
      ...
      <div className="mb-8 bg-white p-6 rounded-lg">
        <h2 className="text-2xl font-bold mb-4 text-gray-800">مقالات تأثير تغير المناخ</h2>
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 overflow-auto bg-black bg-opacity-50 flex items-center justify-center">
          <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl mx-4 p-6 max-h-screen">
            ...
            <form onSubmit={handleSubmit} className="space-y-6 overflow-y-auto max-h-[calc(100vh-12rem)] pr-2 pl-2">
              {/* Image Upload */}
              <div>
                  ...
                  <div className="space-y-1 text-center">
                    ...
                    <div className="flex text-sm text-gray-600 justify-center">
                      <label className="relative cursor-pointer bg-white rounded-md font-medium text-indigo-600 hover:text-indigo-500 focus-within:outline-none">
                        <span>{currentItem && currentItem.image ? 'تغيير الصورة' : 'رفع صورة'}</span>
              ...
              {/* Title */}
              <div>
                <label htmlFor="title" className="block text-right text-gray-700 text-sm font-bold mb-2">عنوان {modalType === 'crop' ? 'المحصول' : 'المقالة'}</label>
                <input
                  type="text"
                  id="title"
              ...
              {(modalType === 'article' || modalType === 'climate' || modalType === 'crop') && (
                <div>
                  <label htmlFor="description" className="block text-right text-gray-700 text-sm font-bold mb-2">وصف مختصر {modalType === 'crop' ? 'للمحصول' : 'للمقالة'}</label>
                  <textarea
                    id="description"
                    rows="3"
                    className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline text-right"
              ...
              {/* Introduction - Only for articles */}
              ...
              {/* Concept Definition - Only for articles */}
              {(modalType === 'article' || modalType === 'climate') && (
                <div>
                  <label htmlFor="conceptDefinition" className="block text-right text-gray-700 text-sm font-bold mb-2">تعريف بالمفهوم أو المشكلة الزراعية</label>
                  <textarea
              ...
              {/* Topic Importance - Only for articles */}
              {(modalType === 'article' || modalType === 'climate') && (
                <div>
                  <label htmlFor="topicImportance" className="block text-right text-gray-700 text-sm font-bold mb-2">أهمية الموضوع</label>
                  <textarea
                    id="topicImportance"
                    rows="3"
                    className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline text-right"
              ...
              {/* Scientific Guidelines - Only for articles */}
              {(modalType === 'article' || modalType === 'climate') && (
                <div>
                  <label htmlFor="scientificGuidelines" className="block text-right text-gray-700 text-sm font-bold mb-2">الخطوات أو النصائح الإرشادية العلمية</label>
                  <textarea
              ...
              {/* Recommendations - Only for articles */}
              {(modalType === 'article' || modalType === 'climate') && (
                <div>
                  <label htmlFor="recommendations" className="block text-right text-gray-700 text-sm font-bold mb-2">التوصيات</label>
                  <textarea
                    id="recommendations"
                    rows="3"
                    className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline text-right"
              ...
              {/* Additional fields for crops */}
              {modalType === 'crop' && (
                <>
                  {/* البيانات العامة عن المحصول */}
                  <div className="p-4 bg-gray-50 rounded-lg mb-4">
                    <h3 className="text-lg font-bold text-gray-700 mb-3 text-right">البيانات العامة عن المحصول</h3>
